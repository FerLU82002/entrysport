...
export const CalendarioReservas = ({ onGestionar, refreshKey = 0 }: Props) => {
  ...
  const semanaLabel = `${format(dias[0], "d 'de' MMM", { locale: es })} – ${format(dias[6], "d 'de' MMM yyyy", { locale: es })}`;

  // Fetch all reservas (calendar filters client-side by week)
  useEffect(() => {
    setCargando(true);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-1.5 rounded-md border border-ink-200 text-ink-600 hover:bg-ink-50 transition-colors"
          >
            <ChevronLeft size={16} strokeWidth={2} />
          </button>
          <span className="text-sm font-medium text-ink-700 min-w-[180px] text-center capitalize">
            {semanaLabel}
          </span>
          <button
            onClick={() => setOffset(p => p + 1)}
          ...
          {offset !== 0 && (
            <button
              onClick={() => setOffset(0)}
              className="text-xs border border-ink-200 rounded-md px-2.5 py-1.5 text-ink-500 hover:text-ink-800 hover:border-ink-300 transition-colors"
            >
              Esta semana
            </button>
          )}
        ...
        <div className="flex items-center gap-4 text-xs text-ink-500">
          {pendientesEnSemana > 0 && (
            <span className="flex items-center gap-1.5 bg-amber-50 text-amber-700 border border-amber-200 rounded-md px-2.5 py-1.5 font-medium">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-500 inline-block" />
              {pendientesEnSemana} pendiente{pendientesEnSemana > 1 ? 's' : ''} esta semana
            </span>
          )}
      ...
      {cargando ? (
        ...
        <div className="overflow-x-auto rounded-lg border border-ink-100 bg-white">
          <div style={{ minWidth: 660 }}>
            ...
            {/* Day headers */}
            <div
              className="grid sticky top-0 bg-white z-20 border-b border-ink-100"
            ...
            >
              ...
              {dias.map((dia, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`py-2.5 text-center border-r border-ink-100 last:border-r-0 relative ${
                      hoy ? 'bg-ink-900' : ''
                    }`}
                  >
                    <p className={`text-[10px] font-semibold uppercase tracking-wide ${
                      hoy ? 'text-ink-400' : 'text-ink-400'
                    }`}>
                      {DIAS[i]}
                    </p>
                    <p className={`text-base font-bold leading-none mt-0.5 ${
                      hoy ? 'text-white' : 'text-ink-800'
                    }`}>
                      {format(dia, 'd')}
                    </p>
            ...
            {HORAS.map(hora => (
              <div
                ...
                style={{ gridTemplateColumns: '56px repeat(7, 1fr)', minHeight: 58 }}
              >
                {/* Hour label — sticky left */}
                <div className="sticky left-0 bg-white z-10 flex items-start justify-end pr-2 pt-1.5 border-r border-ink-100">
                  <span className="text-[11px] text-ink-400 leading-none">{hora}:00</span>
                </div>
                    ...
                    >
                      {visible.map(r => (
                        <button
                          ...
                          className={`w-full text-left rounded text-[11px] px-1.5 py-0.5 leading-tight truncate ${PILL[r.estado]}`}
                        >
                          <span className="block truncate">{nombreCliente(r)}</span>
                      ...
                      {extras > 0 && (
                        <button
                          ...
                          className="text-[10px] text-ink-400 hover:text-ink-700 block w-full text-left px-1"
                        >
                          +{extras} más
                        </button>
                      )}
